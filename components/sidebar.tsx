  ...
  const saveTemplate = async (isDefault: boolean) => {
    ...
    try {
      await saveTemplateToBackend({
        ...serializedData,
        name: isDefault ? `${serializedData.name} (Default)` : serializedData.name,
        isDefault,
      })
      alert(isDefault ? "Template saved as default successfully!" : "Template saved successfully!")
    } catch (error) {
      alert(isDefault ? "Failed to save template as default. Check console for details." : "Failed to save template. Check console for details.")
    }
    */
